        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="px-5 md:px-10 dark:bg-gray-800 sm:rounded-lg">
                <div class="w-full px-6 py-3 bg-white rounded-t-lg shadow-md">
                    ...
                    <form method="GET" id="filter-form" action="{{ route('admin.reservations') }}"
                        class="flex items-end justify-between overflow-x-scroll md:justify-between no-scrollbar">
                        <div class="relative flex divide-x-2">
                            <div class="pr-4">
                                <x-label for="package">Package</x-label>
                                <select name="package" id="package" class="w-auto input-field"
                                    onchange="submitForm()">
                                    <option value="all" {{ request('package') == 'all' ? 'selected' : '' }}>All
                                    </option>
                            ...
                            <div class="px-4">
                                ...
                                <select name="status" id="status" class="w-auto input-field"
                                    onchange="submitForm()">
                                    <option value="0" {{ request('status') == '0' ? 'selected' : '' }}>All</option>
                                    <option value="1" {{ request('status') == '1' ? 'selected' : '' }}>Pending
                                        Payment</option>
                                    <option value="2" {{ request('status') == '2' ? 'selected' : '' }}>Paid (90%)
                                    </option>
                            ...
                            <div class="px-4">
                                ...
                                <select name="date" id="date" class="w-auto input-field"
                                    onchange="submitForm()">
                                    <option value="all" {{ request('date') == 'all' ? 'selected' : '' }}>All
                                    </option>
                                    <option value="week" {{ request('date') == 'week' ? 'selected' : '' }}>Week
                                    </option>
                                    <option value="month" {{ request('date') == 'month' ? 'selected' : '' }}>Month
                                    </option>
                                    <option value="year" {{ request('date') == 'year' ? 'selected' : '' }}>Year
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="relative">
                            <x-input type="text" name="search" class="w-auto mx-1 input-field md:my-1 md:ml-4"
                        ...
                        <x-slot name="tbody">
                            @forelse ($reservations as $index => $reservation)
                                ...
                                <tr class="rounded-lg {{ $rowColor }}">
                                    <td @if ($userArchived) colspan="2" @endif scope="row"
                                        class=" font-medium whitespace-nowrap {{ $userArchived ? '!text-gray-500' : 'text-gray-900' }}">
                                    ...
                                    <td class="text-center text-nowrap">
                                        <span class="{{ $statusClasses }}">{{ $paymentStatus }}</span>
                                    </td>
                            ...
                            @empty
                                <tr class="bg-white border-b">
                                    <td colspan="8" class="py-2 text-xl text-center text-gray-400">No reservations
                                    </td>
                                </tr>
                            @endforelse
                        </x-slot>
