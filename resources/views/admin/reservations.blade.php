            ...
            <div class="p-10 overflow-hidden bg-white shadow-xl dark:bg-gray-800 sm:rounded-lg">
                ...
                <div class="relative overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 rtl:text-right">
                        <thead class="text-base text-gray-700 uppercase bg-primary-light border-b border-gray-300">
                            <tr>
                                <th class="px-6 py-3">
                                    CLient Name
                                </th>
                            ...
                            @forelse ($reservations as $index => $reservation)
                                @php
                                    ...
                                    if ($userArchived) {
                                        ...
                                        $rowColor = $index % 2 == 0 ? 'bg-gray-100' : 'bg-white';
                                    }
                                @endphp
                                <tr class="border-b {{ $rowColor }}">
                                    <th @if($userArchived) colspan="2" @endif scope="row" class="px-6 py-4 font-medium whitespace-nowrap {{ $userArchived ? '!text-gray-500' : 'text-gray-900' }}">
                                        {{ optional($reservation->user)->name ?: 'User Archived' }}
                                    </th>
                                    <td class="px-6 py-4 {{ $userArchived ? 'hidden' : '' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        {{ $reservation->payment_percent == 100 ? 'Paid' : 'Pending Payment' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <a href="{{ route('admin.reservation.show', $reservation->id) }}" class="size-8 mx-1 !p-0 btn-info">
                                            ...
                                            <a href="{{ route('admin.reservation.show', $reservation->id) }}" class="size-8 mx-1 !p-0 btn-danger">
