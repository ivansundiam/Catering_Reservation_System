<div class="inline-flex">
    <button wire:click="showModal" class="nav-link bg-primary !text-white hover:bg-primary-hover !no-underline">
        Sign Up
    </button>
    <x-modal maxWidth="lg" wire:model="showingRegForm">
        <x-authentication-card>
            <x-slot name="logo">
                {{-- <x-authentication-card-logo /> --}}
                <h1 class="text-[2rem] font-sans text-black">Sign Up</h1>
            </x-slot>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="mt-4">
                    ...
                    <x-input id="phone_number" class="block w-full mt-1" type="number" pattern="/^-?\d+\.?\d*$/" min="0" onKeyPress="if(this.value.length==11) return false;" name="phone_number" :value="old('phone_number')" />
                </div>
