<div>
    <button wire:click="show" type="button" class="flex mx-1 md:mx-4 size-8 btn-info justify-center items-center !p-1 my-2">
        ...
        <div class="px-6 py-4 overflow-x-scroll text-sm text-gray-600 bg-gray-100 dark:text-gray-400 no-scrollbar">
            <div class="grid gap-2 py-2 sm:grid-cols-2 lg:grid-cols-4 sm:ml-0">
                <div class="w-full">
                    ...
                    <select wire:model.change="selectedPackage" name="package" id="package" class="w-full input-field">
                            ...
                            <div class="flex justify-center w-full">
                                ...
                                <div class="mx-auto">
                                    ...
                                    <div class="flex">
                                        ...
                                        <div>
                                            <p class="font-normal"> {{ $mostChosenPackageYear->package->name }} <span>({{ $mostChosenPackageYear->package_count }})</span></p>
                                        </div>
                                    </div>
                                    <p class="font-bold">
                                        BEST MONTH: 
                                        <span class="font-normal">{{ date('F', mktime(0, 0, 0,$mostReservedMonth->month , 1)) }} ({{ $mostReservedMonth->reservation_count }})</span>
                                    </p>
                                    <div class="flex">
                                        <p class="mr-1 font-bold">TOTAL EARNINGS: </p>
                                        <div>
                                            <p class="font-normal">₱{{ number_format($totalEarnings, 2, '.', ',') }}</p>
                                        </div>
                                    </div>
                                </div>
                                @else
                                    <div class="mx-auto">
                                        <p class="font-bold">TOTAL - Month of <span>{{  date('F', mktime(0, 0, 0, $selectedMonth, 1)) }}</span></p>
                                        <div class="flex">
                                            <p class="mr-1 font-bold">RESERVATIONS: </p>
                                            <span class="font-normal"> {{ $reservationsCount }}</span>
                                        </div>
                                        <div class="flex">
                                            <p class="mr-1 font-bold">BEST SELLER PACKAGE: </p>
                                            <div>
                                                <p class="font-normal"> {{ $mostChosenPackageMonth->package->name }} <span>({{ $mostChosenPackageMonth->package_count }})</span></p>
                                            </div>
                                        </div>
                                        <div class="flex">
                                            <p class="mr-1 font-bold">TOTAL EARNINGS: </p>
