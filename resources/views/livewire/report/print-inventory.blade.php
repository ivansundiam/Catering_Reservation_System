<div>
    ...
    <x-modal wire:model="showing" maxWidth="5xl">
        ...
        <div class="px-6 py-4 overflow-x-scroll text-sm text-gray-600 bg-gray-100 dark:text-gray-400 no-scrollbar">
            <div class="grid gap-2 py-2 sm:grid-cols-2 lg:grid-cols-4 sm:ml-0">
                <div class="w-full">
                    ...
                    <select wire:model.change="selectedCategory" name="category" id="category"
                        class="w-full input-field">
                        <option value="all" {{ request('category') == 'all' ? 'selected' : '' }}>All</option>
                            <option value="CATERING EQUIPMENT"
                                {{ request('category') == 'CATERING EQUIPMENT' ? 'selected' : '' }}>{{ __('CATERING EQUIPMENT') }}
                            </option>
                            <option value="CHAIRS & TABLES"
                                {{ request('category') == 'CHAIRS & TABLES' ? 'selected' : '' }}>{{ __('CHAIRS & TABLES') }}
                            </option>
                    </select>
                </div>
            ...
            <div class="grid mx-auto">
                <span class="mx-auto my-5 text-lg font-semibold capitalize">Inventory report preview</span>
                                    ...
                                    <tbody>
                                        @forelse ($inventory as $item)
                                            <tr>
                                                ...
                                                <td class="pl-3">
                                                    {{ number_format(($item->inventory->price * $item->quantity_rented), 2, '.', ',') }}
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="6" class="p-3 text-center">No rented items</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            ...
                            <div class="flex w-full">
                                @if ($selectedDate == 'annually')
                                    <div>
                                        ...
                                        <div class="flex">
                                            <p class="mr-1 font-bold">ITEMS RENTED: </p>
                                            <span class="font-normal"> {{ $inventoryCount }}</span>
                                        </div>
                                        <div class="flex">
                                            <p class="mr-1 font-bold">BEST SELLER PACKAGE: </p>
                                            <div>
                                                @if ($mostRentedItemYear)
                                                    <p class="font-normal">
                                                        {{ $mostRentedItemYear->inventory->item_name }}
                                                        <span>({{ $mostRentedItemYear->item_count }})</span>
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
                                        <p class="font-bold">
                                            BEST MONTH:
                                            <span
                                                class="font-normal">{{ date('F', mktime(0, 0, 0, $mostMonthWithRent->month, 1)) }}
                                                ({{ $mostMonthWithRent->item_count }})</span>
                                        </p>
                                        <div class="flex">
                                ...
                                @else
                                    <div>
                                        ...
                                        <div class="flex">
                                            <p class="mr-1 font-bold">MOST RENTED ITEM: </p>
                                            @if ($mostRentedItemMonth)
                                                <p class="font-normal">
                                                    {{ $mostRentedItemMonth->inventory->item_name }}
                                                    <span>({{ $mostRentedItemMonth->item_count }})</span>
                                                </p>
                                            @endif
                                        </div>
                                        <div class="flex">
        ...
        <div class="flex flex-row justify-end px-6 py-4 bg-gray-100 dark:bg-gray-800 text-end">
            ...
            <form action="{{ route('inventory-report-pdf') }}" method="POST" target="_blank">
                @csrf
                <input type="hidden" name="reportDetails" value="{{ json_encode($reportDetails) }}">
                <button type="submit" class="ml-2 btn-info" {{ $noItems ? 'disabled' : '' }}>Print</button>
            </form>
        </div>
    </x-modal>
</div>
